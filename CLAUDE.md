...
- **새 기본 영역 자산(훅·스킬·설정 키)을 만들 때는 구현 전에 설계(무엇을·왜·어떻게 검증할지)를 채팅에 제시하고 승인받은 뒤 만든다.** 기존 자산 수정(버그 수정, 문구 정정)에는 안 걸린다 — 새로 생기는 것에만.
...
- 작업 중 원본 이슈에서 파생된 작업은 평행한 새 이슈로 두지 않고 sub-issue로 연결한다 — 새로 열 땐 `gh issue create --parent <원본번호>`, 이미 있는 이슈를 묶을 땐 `gh issue edit <원본번호> --add-sub-issue <파생번호>`(개인 저장소에서도 동작 확인, 2026-09-13).
