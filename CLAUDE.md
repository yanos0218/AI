...
- 예외
  - 기본 영역의 오탈자·깨진 링크처럼 동작에 영향 없는 수정은 확인 프롬프트에서 사유를 말하고 승인받으면 된다.
- **새 기본 영역 자산(훅·스킬·설정 키)을 만들 때는 구현 전에 설계(무엇을·왜·어떻게 검증할지)를 채팅에 제시하고 승인받은 뒤 만든다.**
  - 기존 자산 수정(버그 수정, 문구 정정)에는 안 걸린다
    - 새로 생기는 것에만.
...
- 링크
  - 파일·폴더 경로는 항상 클릭되는 링크로 쓴다.
  - 과거 결정을 가리킬 땐 해당 GitHub Issue 링크(closed 포함, PROGRESS.md엔 더 이상 항목별 앵커 없음).
...
- 여러 세션에 걸치는 이슈는 **작업할 때마다 댓글로 진행 상황을 남긴다.**
  - 끝났을 때 요약 하나만 쓰지 않는다
    - 중간에 뭘 했는지가 댓글 이력으로 남아야 한다.
  - 끝나면 커밋에 `Refs #N`만 쓰고, 검증 끝난 뒤 `gh issue close`로 직접 닫는다(push 순간 자동으로 닫혀 검증 전에 닫히는 위험 방지, [Issue #122](https://github.com/yanos0218/AI/issues/122)).
