...
- 스킬 발동 테스트는 `bash tools/test-skill.sh <스킬 폴더> <시나리오 git 저장소> "<사용자 말>"` — 시나리오 저장소의 `.claude/skills/`에 넣고 새 세션(`claude -p`, 기본 Sonnet)으로 돌려 발동·도구 호출·비용을 본다. 스크립트가 맨 먼저 `claude plugin validate`로 구조를 공짜로 검사한 뒤 실제 시험을 돈다(2026-09-12). 같은 세션 안에서 스킬을 직접 호출하는 것은 발동 테스트가 아니다. 세션이 "했다"고 말한 것과 실제 파일 변경을 대조한다.
...
- 줄 수 상한(`tools/check-docs.sh`가 CI에서 검사): README 120, 이 파일 60, `base/claude-md/CLAUDE.md` 200, HANDOFF 60, PROGRESS 300, SKILL.md 80. 넘으면 줄이는 게 아니라 **분리**한다 — README는 `docs/`로. PROGRESS는 §0 표만 담으므로 보통 넘지 않는다.
...
- 링크: 파일·폴더 경로는 항상 클릭되는 링크로 쓴다. 과거 결정을 가리킬 땐 해당 GitHub Issue 링크(closed 포함) — PROGRESS.md엔 더 이상 항목별 앵커가 없다. 코드 블록 안에는 링크가 안 되므로 구조 목록은 표로.
