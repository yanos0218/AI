...
- 셸 스크립트 기준은 `npx shellcheck -S warning`(기존 13개 스크립트는 이 기준으로 실제 위험 0건 확인, 2026-09-13 — `--enable=all`이 더 잡는 건 전부 스타일). **새로 쓰는 스크립트만** `${var}`·`[[ ]]`로 쓴다. 기존 스크립트를 스타일 때문에 다시 쓰지 않는다.
