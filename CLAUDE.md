...
- 파일마다 역할이 하나다. README = 입구(구조·설치 요약·링크). HANDOFF = 지금 상황(현재 상태는 **4줄 이하 + 링크**, 상세는 PROGRESS §0). PROGRESS = 단계·배포·할 일. 상세 주제는 `docs/<주제>.md` 하나씩.
...
- 작업 중 발견한 문제는 GitHub Issue로 열고 보드 항목에 `#N`을 적는다(C-34 시험, 2026-09-09~). 계획·상태는 보드.
