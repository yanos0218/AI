...
| 5 | Issues | 문제·할 일을 잃지 않기 위해 | 개인용 저장소: 문제(`bug`) + 계획된 할 일(`task`) 모두 Issue로, 템플릿은 안 만든다(1인). 팀·협업 저장소는 별도 — 실제로 그런 저장소가 생기면 `repo-setup` 조사 단계에서 다시 정한다(이슈 템플릿·담당자·마일스톤이 필요해질 가능성이 큼). 커밋에 `#N`, 끝나면 `Closes #N` |
