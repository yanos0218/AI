...
| --- | --- | --- | --- | --- | --- | --- |
| 전역 지침 `base/claude-md/CLAUDE.md` | 기본 | ✓ v0.4.0+미릴리즈 §2·§5 | ✓ v0.4.0+미릴리즈 (2026-09-09) | ✗ | ✗ (Project instructions) | [C-16](#c-16)/17 |
...
- [~] <a id="c-53"></a>C-53 릴리즈 전 자격증명 점검 — 초안 3건(전역 §7 한 줄, dev-release 0단계, `check-install.sh` VS Code 확장 대조) + 발동 시험 통과(미인증 상태에서 편집 전 `gh auth status` → 멈추고 `gh auth login` 안내, 토큰 요구 없음). 시험 중 `test-skill.sh` 버그 2건 수정([#3](https://github.com/yanos0218/AI/issues/3)). 상세 [docs/progress/C-53.md](progress/C-53.md). **승격은 사용자 요청 시** (2026-09-09)
