...
- 저장소 v0.7.0 + [Unreleased](../CHANGELOG.md#unreleased)(셸 스크립트 13개 `${var}`·`[[ ]]` 통일, self-audit 스킬 초안+session-start-check 3번째 확인 — 발동 시험 전). CI(`lint.yml`) 초록. 배포 현황은 [PROGRESS.md §0](PROGRESS.md#0-자산-현황--단계와-배포-상태) 표가 원본 — Windows는 v0.7.0, Mac·웹은 v0.6.0, Linux는 미반영.
